...
func (parser *Parser) CheckRealPackagePath(packagePath string) string {
	...
	if pkgRealpath == "" {
		...
		for _, path := range gopathsList {
			if evalutedPath, err := filepath.EvalSymlinks(filepath.Join(path, "src", vendoringPath, packagePath)); err == nil {
				if _, err := os.Stat(evalutedPath); err == nil {
					pkgRealpath = evalutedPath
					break
				}
		...
		for _, path := range gopathsList {
			if evalutedPath, err := filepath.EvalSymlinks(filepath.Join(path, "src", parser.BasePath, packagePath)); err == nil {
				if _, err := os.Stat(evalutedPath); err == nil {
					pkgRealpath = evalutedPath
					break
				}
